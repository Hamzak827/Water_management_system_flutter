...
class _DeliveryboyModalState extends State<DeliveryboyModal> {
  ...
  void initState() {
    ...
    if (widget.isEditing && widget.deliveryboy != null) {
      ...
      _passwordController=TextEditingController(text: widget.deliveryboy!['Password']);
      _phoneController = TextEditingController(text: widget.deliveryboy!['Phone']);
      _assignedareaController = TextEditingController(text: widget.deliveryboy!['AssignedArea']);
      _cnicController = TextEditingController(text: widget.deliveryboy!['CNIC']);
...
void _submitDeliveryboy() async {
 ...
 Map<String, dynamic> deliveryboyData = {
    ...
    try {
       ...
       bool success = false;
    if (widget.isEditing) {
      success = await AuthService().updateDeliveryboyData(widget.deliveryboyId!, deliveryboyData);
    } else {
      ...
      if (success) {
        ...
        DialogUtil.showErrorMessage(context,widget.isEditing ? 'Deliveryboy Updated Successfully' : 'Deliveryboy Added Successfully');
      }
    } catch (e) {
...
Widget build(BuildContext context) {
  return Scaffold(
    appBar: AppBar(
      title: Text(widget.isEditing ? "Edit Delivery Boy" : "Add Delivery Boy"),
      leading: IconButton(
