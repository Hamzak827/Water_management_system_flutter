...
class DeliveryboyOrderScreen extends StatefulWidget {
  ...
  static const routeName = '/delivery-boy-order-screen'; // Add a route name for navigation

 @override
  _DeliveryboyOrderScreenState createState() =>
      _DeliveryboyOrderScreenState();
}



class _DeliveryboyOrderScreenState
    extends State<DeliveryboyOrderScreen> {
...
Padding(
  ...
  child: Row(
    children: [
      ...
      if (['Processing', 'Delivered', 'Canceled'].contains(customer['Status']))
  ...
  child: DropdownButton<String>(
      ...
      'Canceled'
        ...
        .map((status) => DropdownMenuItem(
              ...
              child: Container(
                ...
                padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(
                  color: _getStatusColor(status), // Set background color
                  borderRadius: BorderRadius.circular(8.0), // Apply rounded corners
                ),
                child: Text(
                  status,
    ...
    onChanged: (newValue) async {
      if (newValue == null || newValue == customer['Status']) return; // No change

      String originalStatus = customer['Status']; // Store original status
      bool? result;
      ...
      if (newValue == "Canceled") {
        result = await showDialog(
          ...
          builder: (BuildContext context) {
            return CanceledStatusModal(order: customer);
          },
        );
        ...
        result = await showDialog(
          context: context,
          builder: (BuildContext context) {
            return DeliveredStatusModal(order: customer);
          },
        );
      } else {
        result = true; // Directly allow change for "Out For Delivery"
      }
