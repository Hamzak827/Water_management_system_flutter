...
class SuperAdminPrepaidTokenScreen extends StatefulWidget {
  const SuperAdminPrepaidTokenScreen({super.key});

  static const routeName = '/super-admin-prepaidtoken-screen';

  @override
  State<SuperAdminPrepaidTokenScreen> createState() => _SuperAdminPrepaidTokenScreenState();
}

class _SuperAdminPrepaidTokenScreenState extends State<SuperAdminPrepaidTokenScreen> {
  ...
  void _filterCustomers() {
    filteredCustomers = customers.where((customer) {
      return customer['PrepaidTokens'] != null && customer['PrepaidTokens'].isNotEmpty;
    }).toList();
  }
            ...
            children: prepaidTokens.map((token) {
              return Column(
                ...
                children: [
                  ...
                  Text("Start Serial Number: ${token['serialNumberStarting']}"),
...
void _showUnusedTokensDialog(Map<String, dynamic> token) {
  ...
  List<int> unusedTokens = List.generate(end - start + 1, (index) => start + index);
  ...
  final endIndex = (startIndex + _itemsPerPage).clamp(0, filteredCustomers.length);
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (filteredCustomers.isEmpty) {
              return const Center(child: Text('No customers with tokens found.'));
            }
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                     ...
                     final token = (customer['PrepaidTokens'] != null && customer['PrepaidTokens'].isNotEmpty)
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                // Customer ID
                                _buildRow("Customer ID:", customer['CustomerID'].toString()),

                                // Active Serial Number
                               _buildRow("Active Serial:", token != null ? "${token['serialNumberStarting']} - ${token['serialNumberEnding']}" : "N/A"),


                                // Used Tokens
                                _buildRow("Used Tokens:", customer['UsedPrepaidTokens']?.length.toString() ?? '0'),

                                // Unused Tokens
                              _buildRow(
  "Unused Tokens:",
  ((token['numberoftokens'] ?? 0) - ((customer['UsedPrepaidTokens']?.length ?? 0))).toString(),
),


                                // Token Book Price
                                _buildRow("Token Book Price:", "\$${token['PricePerBook'].toString()}"),

                                // Price Per Book
                                _buildRow("Price Per Book:", "\$${customer['PricePerBook'].toString()}"),
...
Wrap(
  ...
  runSpacing: 8, // Vertical spacing when wrapping to a new line
  children: [
    // Used Button
    ...
    SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        onPressed: () async {
          if (customer['PrepaidTokens'] != null && customer['PrepaidTokens'].isNotEmpty) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text("No prepaid tokens found.")),
            );
          }
        },
    ...
    SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        onPressed: () async {
          if (customer['UsedPrepaidTokens'] != null && customer['UsedPrepaidTokens'].isNotEmpty) {
            _showUsedTokensDialog(List<Map<String, dynamic>>.from(customer['UsedPrepaidTokens']));
          } else {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text("No used tokens found.")),
            );
          }
        },
    ...
    SizedBox(
     ...
     onPressed: () {
  if (customer['PrepaidTokens'] != null && customer['PrepaidTokens'].isNotEmpty) {
    _showPrepaidTokensDialog(List<Map<String, dynamic>>.from(customer['PrepaidTokens']));
  } else {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text("No prepaid tokens found.")),
    );
    ...
    SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        onPressed: () async {
          if (customer['PrepaidTokens'] != null && customer['PrepaidTokens'].isNotEmpty) {
            _showUnusedTokensDialog(customer['PrepaidTokens'].last);
          } else {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text("No prepaid tokens found.")),
            );
    ...
    SingleChildScrollView(
      ...
      child: Row(
        children: [
          ...
          Builder(
            builder: (context) {
              int totalPages = (((filteredCustomers.length) - 1) / _itemsPerPage).ceil();
    ...
    child: FloatingActionButton(
      ...
      onPressed: () async {
        final result = await showDialog(
          context: context,
          builder: (BuildContext context) {
            return PrepaidTokenModal(isEditing:false, customerId: '',);
          },
        );
        if (result == true) {
          _fetchCustomers();
